...
function rpc(request, callback) {
  ...
  var errorMessage = 'Solarcoin JSON-RPC: ';
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        var exceededError = new Error('Solarcoin JSON-RPC: ' + buf.toString('utf8'));
...
RpcClient.callspec = {
  ...
  sendToAddress: 'str float str str str',
  setAccount: '',
